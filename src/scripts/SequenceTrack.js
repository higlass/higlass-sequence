  ...
  class SequenceTrackClass extends HGC.tracks.BarTrack {
    constructor(context, options) {
      ...
      letters.forEach(letter => {
        this.pixiTexts[letter] = new HGC.libraries.PIXI.Text(letter, this.options.textOption);
        this.pixiTexts[letter].updateText();
        // We get sharper edges if we scale down a large letter
        this.letterWidths[letter] = this.pixiTexts[letter].getBounds().width / 2;
        this.letterHeights[letter] = this.pixiTexts[letter].getBounds().height / 2;
        this.maxLetterWidth = Math.max(this.letterWidths[letter], this.maxLetterWidth);
        this.maxLetterHeight = Math.max(this.letterHeights[letter], this.maxLetterHeight);
    ...
    updateOptions(newOptions){
      ...
      this.notificationText = new HGC.libraries.PIXI.Text(newOptions.notificationText, {
        fontSize: '13px',
        fontFamily: 'Arial',
        fill : 0x333333
      });
    ...
    createColorAndLetterData(tile) {
      ...
      for (let j = 0; j < matrix.length; j++) {
        ...
        if(sum === 1){
          ...
          const sumRelevantEntries = relevantEntries.reduce((pv, cv) => pv + cv, 0);

          if(sumRelevantEntries === 1){ // one letter appears more often than any other
            ...
            const ind = nonZeroInd[Math.floor(Math.random() * nonZeroInd.length)];
            columnColors = {
              letter: this.getLetterFromArray(ind),
              color: this.colorScale[ind]
            }
          ...
          interpolatedColor.r = this.colorScaleRgb.reduce(function (res, cv, ind) {
            return res + weigths[ind] * cv.r
          }, 0);

          interpolatedColor.g = this.colorScaleRgb.reduce(function (res, cv, ind) {
            return res + weigths[ind] * cv.g
          }, 0);

          interpolatedColor.b = this.colorScaleRgb.reduce(function (res, cv, ind) {
    ...
    drawTextSequence(tileX, tileWidth, tile) {
      ...
      const width = (this._xScale(tileX + tileWidth) - this._xScale(tileX)) / matrix.length;
      ...
      for (let j = 0; j < matrix.length; j++) { // jth vertical bar in the graph
        ...
        if(this.options.barBorder){
          tile.borderGraphics.lineStyle(1, this.barBorderColor)
            .moveTo(txStart, 0)
    ...
    drawVerticalBars(tileX, tileWidth, tile) {
      ...
      for (let j = 0; j < matrix.length; j++) { // jth vertical bar in the graph
        ...
        this.addSVGInfo(tile, x, 0, width, trackHeight, nucleotide.color, tile.texts[j].letter);
      ...
      const texture = pixiRenderer.generateTexture(
        tile.tempGraphics, HGC.libraries.PIXI.SCALE_MODES.NEAREST
      );
    ...
    calculateTilesFromResolution (
      ...
      if(this.options.extendedPreloading){
        ...
        const lastTile = Math.min(Math.max.apply(null, tileRange) + 1, Math.ceil(maxX / tileWidth));
